...
```{r Mapping}
maui_poverty_map <- ggplot(maui_poverty) +
  geom_sf_interactive(
    aes(
      fill = percent_povertyE,
      tooltip = paste(NAME, ": ", percent_povertyE, "%"),
...
girafe(ggobj = maui_poverty_map, width = 700, height = 400)
```
...
oahu_education <- get_acs(
  geography = "tract",
  variables = c(percent_bachelors = "DP02_0068P"),
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
...
```{r, message=FALSE, warning=FALSE, results='hide'}
oahu_education_map <- ggplot(oahu_education) +
  geom_sf_interactive(
    aes(
      fill = percent_povertyE,
      tooltip = paste(NAME, ": ", percent_povertyE, "%"),
...
oahu_education_2018_2023 <- map_dfr(years, ~{
  get_acs(
    geography = "county",
    variables = c(percent_bachelors = "DP02_0067PE"),
    state = "HI",
    county = "Honolulu",
    year = .x,
    survey = "acs5"
...
oahu_education_2018_2023$year <- as.numeric(oahu_education_2018_2023$year)

ggplot(oahu_education_2018_2023, aes(x = year, y = estimate)) +
  geom_ribbon(aes(ymax = estimate + moe, ymin = estimate - moe),
              fill = "#2b83ba", alpha = 0.3) +
  geom_line(size = 1.2, color = "#2b83ba") +
  geom_point(size = 3, color = "#2b83ba") +
  scale_y_continuous(labels = percent_format(scale = 1)) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Honolulu County: Bachelor's Degree or Higher (ACS 5-Year Estimates)",
    subtitle = "2018–2023",
    x = "ACS Release Year (5-year window)",
    y = "Percent with Bachelor's or Higher",
    caption = "Source: U.S. Census Bureau, American Community Survey\nShaded area represents margin of error"
  )
```
...
ggplot(oahu_family_poverty_college_education, aes(x = percent_povertyE,y = percent_BAE)) +
  ...
  stat_cor() + 
  theme_minimal() + 
  labs(title = "Does Poverty Affect Higher Education, Honolulu County",
       subtitle = "ACS 5-year, 2023 release",
       x = "Percent of Families in Poverty",
       y = "Percent with a BA or higher",
       caption = "Shaded area represents margin of error")
